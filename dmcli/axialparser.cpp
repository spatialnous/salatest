...
void AxialParser::parse(size_t argc, char **argv) {
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "-xl") == 0) {
            ...
            if (m_runAnalysis) {
                throw dmcli::CommandLineException("-xa can only be used once");
            }
            m_radii = dmcli::parseRadiusList(argv[i]);
            m_runAnalysis = true;
    ...
    if (!runAllLines() && !runFewestLines() && !runUnlink() && !runAnalysis()) {
        throw dmcli::CommandLineException("No axial analysis mode present");
    }
}
