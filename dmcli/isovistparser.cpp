...
void IsovistParser::parse(size_t argc, char **argv) {
    ...
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "-ii") == 0) {
            if (!isovistFile.empty()) {
                throw dmcli::CommandLineException("-ii cannot be used together with -if");
            }
            ENFORCE_ARGUMENT("-ii", i)
            m_isovists.push_back(EntityParsing::parseIsovist(argv[i]));
        } else if (std::strcmp(argv[i], "-if") == 0) {
            if (!isovistFile.empty()) {
                throw dmcli::CommandLineException("-if can only be used once");
            }
            if (!m_isovists.empty()) {
                throw dmcli::CommandLineException("-if cannot be used together with -ii");
            }
    ...
    if (m_isovists.empty()) {
        throw dmcli::CommandLineException("No isovists defined. Use -ii or -if");
    }
}
