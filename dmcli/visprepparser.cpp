...
void VisPrepParser::parse(size_t argc, char **argv) {
    ...
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp("-pg", argv[i]) == 0) {
            if (m_grid >= 0) {
                throw dmcli::CommandLineException("-pg can only be used once");
            }
            ENFORCE_ARGUMENT("-pg", i)
            m_grid = std::atof(argv[i]);
            if (m_grid <= 0) {
                throw dmcli::CommandLineException(std::string("-pg must be a number >0, got ") +
                                                  argv[i]);
            }
        } else if (std::strcmp("-pp", argv[i]) == 0) {
            if (!pointFile.empty()) {
                throw dmcli::CommandLineException("-pp cannot be used together with -pf");
            }
            ...
            if (!points.empty()) {
                throw dmcli::CommandLineException("-pf cannot be used together with -pp");
            }
            ...
            if (m_maxVisibility == 0.0) {
                ...
                throw dmcli::CommandLineException(message.str());
            }
        } else if (std::strcmp("-pb", argv[i]) == 0) {
            m_boundaryGraph = true;
        } else if (std::strcmp("-pm", argv[i]) == 0) {
            if (getUnmakeGraph()) {
                throw dmcli::CommandLineException("-pm cannot be used together with -pu");
            }
            m_makeGraph = true;
        } else if (std::strcmp("-pu", argv[i]) == 0) {
            if (getMakeGraph()) {
                throw dmcli::CommandLineException("-pu cannot be used together with -pm");
            }
            m_unmakeGraph = true;
    ...
    if (!getMakeGraph() && !getUnmakeGraph() && m_grid <= 0 && pointFile.empty() &&
        points.empty()) {
        throw dmcli::CommandLineException("Nothing to do");
    }

    if (m_grid > 0 && getMakeGraph() && pointFile.empty() && points.empty()) {
        throw dmcli::CommandLineException("Creating a graph for an unfilled grid is not possible. "
    ...
    if (m_removeLinksWhenUnmaking && !m_unmakeGraph) {
        throw dmcli::CommandLineException("-pl can only be used together with -pu");
    }
}
