...
void VisPrepParser::parse(size_t argc, char **argv) {
    ...
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp("-pg", argv[i]) == 0) {
            if (m_grid >= 0) {
                throw dmpcli::CommandLineException("-pg can only be used once");
            }
            ENFORCE_ARGUMENT("-pg", i)
            m_grid = std::atof(argv[i]);
            if (m_grid <= 0) {
                throw dmpcli::CommandLineException(std::string("-pg must be a number >0, got ") +
                                                   argv[i]);
            }
        } else if (std::strcmp("-pp", argv[i]) == 0) {
            if (!pointFile.empty()) {
                throw dmpcli::CommandLineException("-pp cannot be used together with -pf");
            }
            ...
            if (!points.empty()) {
                throw dmpcli::CommandLineException("-pf cannot be used together with -pp");
            }
            ...
            if (getUnmakeGraph()) {
                throw dmpcli::CommandLineException("-pm cannot be used together with -pu");
            }
            m_makeGraph = true;
        } else if (std::strcmp("-pu", argv[i]) == 0) {
            if (getMakeGraph()) {
                throw dmpcli::CommandLineException("-pu cannot be used together with -pm");
            }
            m_unmakeGraph = true;
    ...
    if (!getMakeGraph() && !getUnmakeGraph() && m_grid <= 0 && pointFile.empty() &&
        points.empty()) {
        throw dmpcli::CommandLineException("Nothing to do");
    }

    if (m_grid > 0 && getMakeGraph() && pointFile.empty() && points.empty()) {
        throw dmpcli::CommandLineException("Creating a graph for an unfilled grid is not possible. "
    ...
    if (m_removeLinksWhenUnmaking && !m_unmakeGraph) {
        throw dmpcli::CommandLineException("-pl can only be used together with -pu");
    }
}
...
void VisPrepParser::run(const CommandLineParser &clp, IPerformanceSink &perfWriter) const {
    ...
    if (m_grid > 0) {
        ...
        if (m_grid > gp.getMax() || m_grid < gp.getMin()) {
            ...
            throw genlib::RuntimeException(message.str());
        }
