...
void MapConvertParser::parse(size_t argc, char **argv) {
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp("-co", argv[i]) == 0) {
            ...
            if (std::strcmp(argv[i], "drawing") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid map output (-co) type: ") +
                                                   argv[i]);
            }
            ...
            if (!dmpcli::has_only_digits_dots(argv[i])) {
                throw dmpcli::CommandLineException(std::string("-crsl must be a number >0, got ") +
                                                   argv[i]);
            }
    ...
    if (m_outMapType == ShapeMap::EMPTYMAP) {
        throw dmpcli::CommandLineException("A valid output map type (-co) is required");
    }

    if (m_outMapName == "") {
        throw dmpcli::CommandLineException("A valid output map name (-con) is required");
    }
}
