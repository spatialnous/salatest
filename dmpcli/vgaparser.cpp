...
void VgaParser::parse(size_t argc, char *argv[]) {
    for (size_t i = 1; i < argc;) {
        ...
        if (std::strcmp("-vm", argv[i]) == 0) {
            ...
            if (std::strcmp(argv[i], "isovist") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid VGA mode: ") + argv[i]);
            }
    ...
    if (m_vgaMode == VgaMode::VISBILITY && m_globalMeasures) {
        ...
        if (m_radius != "n" && !dmpcli::has_only_digits(m_radius)) {
            throw dmpcli::CommandLineException(
        ...
        if (m_radius.empty()) {
            throw dmpcli::CommandLineException("Metric vga requires a radius, use -vr <radius>");
        }
    }
}
...
void VgaParser::run(const CommandLineParser &clp, IPerformanceSink &perfWriter) const {
    ...
    case VgaParser::VgaMode::THRU_VISION:
        ...
        break;
    default:
        throw dmpcli::SetupCheckException("Unsupported VGA mode");
    }
