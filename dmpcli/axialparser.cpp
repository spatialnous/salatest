...
void AxialParser::parse(size_t argc, char **argv) {
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "-xl") == 0) {
            ...
            if (m_runAnalysis) {
                throw dmpcli::CommandLineException("-xa can only be used once");
            }
    ...
    if (!runAllLines() && !runFewestLines() && !runUnlink() && !runAnalysis()) {
        throw dmpcli::CommandLineException("No axial analysis mode present");
    }
}
...
void AxialParser::run(const CommandLineParser &clp, IPerformanceSink &perfWriter) const {
    ...
    if (runFewestLines()) {
        ...
        if (!metaGraph.hasAllLineMap()) {
            throw genlib::RuntimeException("All line map must be constructed before fewest "
