...
void SegmentShortestPathParser::parse(size_t argc, char **argv) {
    ...
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp("-sspo", argv[i]) == 0) {
            if (!originPoint.empty()) {
                throw dmpcli::CommandLineException("-sspo can only be provided once");
            }
        ...
        if (std::strcmp("-sspd", argv[i]) == 0) {
            if (!destinationPoint.empty()) {
                throw dmpcli::CommandLineException("-sspd can only be provided once");
            }
            ...
            if (std::strcmp(argv[i], "tulip") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid step type: ") + argv[i]);
            }
        }
    }

    if (originPoint.empty() || destinationPoint.empty()) {
        throw dmpcli::CommandLineException("Both -sspo and -sspd must be provided");
    }
    ...
    if (m_stepType == StepType::NONE) {
        throw dmpcli::CommandLineException("Step depth type (-sspt) must be provided");
    }
}
...
void SegmentShortestPathParser::run(const CommandLineParser &clp,
    ...
    if (!graphRegion.contains(m_originPoint)) {
        throw genlib::RuntimeException("Origin point outside of target region");
    }
    if (!graphRegion.contains(m_destinationPoint)) {
        throw genlib::RuntimeException("Destination point outside of target region");
    }
    ...
    default: {
        throw dmpcli::SetupCheckException("Error, unsupported step type");
    }
    }
