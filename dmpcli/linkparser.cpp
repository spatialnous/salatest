...
void LinkParser::parse(size_t argc, char *argv[]) {
    for (size_t i = 1; i < argc;) {
        if (std::strcmp("-lmt", argv[i]) == 0) {
            ...
            if (std::strcmp(argv[i], "pointmaps") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid LINK map type group: ") +
                                                   argv[i]);
            }
            ...
            if (std::strcmp(argv[i], "link") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid LINK mode: ") + argv[i]);
            }
            ...
            if (std::strcmp(argv[i], "coords") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid LINK type: ") + argv[i]);
            }
        } else if (std::strcmp("-lf", argv[i]) == 0) {
            if (!m_linksFile.empty()) {
                throw dmpcli::CommandLineException("-lf can only be used once at the moment");
            } else if (m_manualLinks.size() != 0) {
                throw dmpcli::CommandLineException("-lf can not be used in conjunction with -lnk");
            }
    ...
    if (m_manualLinks.size() == 0 && m_linksFile.empty()) {
        throw dmpcli::CommandLineException("one of -lf or -lnk must be provided");
    }
}
