    ...
    for (size_t i = 1; i < argc;) {
        ...
        if (std::strcmp("-ei", argv[i]) == 0) {
            if (m_exportMapIdx.has_value()) {
                throw dmpcli::CommandLineException("-ei can only be used once");
            }
            ENFORCE_ARGUMENT("-ei", i)
            if (!dmpcli::has_only_digits(argv[i])) {
                throw dmpcli::CommandLineException(std::string("-ei must be an integer >0, got ") +
                                                   argv[i]);
            }
            ...
            if (std::strcmp(argv[i], "pointmap-data-csv") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid EXPORT mode: ") + argv[i]);
            }
        }
        ++i;
    }
...
PointMapDM &ExportParser::getSelectedOrDisplayedPointMap(MetaGraphDM &mgraph) const {
    if (mgraph.getPointMaps().size() == 0) {
        throw dmpcli::CommandLineException("No pointmaps available.");
    }
    if (m_exportMapIdx.has_value()) {
...
ShapeGraphDM &ExportParser::getSelectedOrDisplayedShapeGraph(MetaGraphDM &mgraph) const {
    if (mgraph.getShapeGraphs().size() == 0) {
        throw dmpcli::CommandLineException("No shapegraphs available.");
    }
    if (m_exportMapIdx.has_value()) {
...
void ExportParser::run(const CommandLineParser &clp, IPerformanceSink &perfWriter) const {
    ...
    default: {
        throw dmpcli::SetupCheckException("Error, unsupported export mode");
    }
    }
}
