...
void SegmentParser::parse(size_t argc, char **argv) {
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp("-st", argv[i]) == 0) {
            ...
            if (std::strcmp(argv[i], "tulip") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid SEGMENT mode: ") + argv[i]);
            }
            ...
            if (std::strcmp(argv[i], "steps") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid SEGMENT radius type: ") +
                                                   argv[i]);
            }
            ...
            m_radii = dmpcli::parseRadiusList(argv[i]);
    ...
    if (getAnalysisType() == InAnalysisType::NONE) {
        throw dmpcli::CommandLineException("No analysis type given");
    }

    if (getRadii().empty()) {
        throw dmpcli::CommandLineException("At least one radius must be provided");
    }

    if (getAnalysisType() == InAnalysisType::ANGULAR_TULIP &&
        getRadiusType() == InRadiusType::NONE) {
        throw dmpcli::CommandLineException("Radius type is required for tulip analysis");
    }

    if (getAnalysisType() == InAnalysisType::ANGULAR_TULIP && getTulipBins() == 0) {
        throw dmpcli::CommandLineException("Tulip bins are required for tulip analysis");
    }
...
void SegmentParser::run(const CommandLineParser &clp, IPerformanceSink &perfWriter) const {
    ...
    if (!getAttribute().empty()) {
        ...
        if (options.weightedMeasureCol == -1) {
            throw genlib::RuntimeException("Given attribute (" + getAttribute() +
    ...
    case InAnalysisType::NONE:
        throw genlib::RuntimeException("No segment analysis type given");
    }
    std::cout << "ok\n" << std::flush;
