    ...
    for (size_t i = 1; i < argc; ++i) {
        if (strcmp("-if", argv[i]) == 0) {
            ...
            if (std::strcmp(argv[i], "drawing") == 0) {
                m_importMapType = sala::ImportType::DRAWINGMAP;
            } else if (std::strcmp(argv[i], "data") == 0) {
                m_importMapType = sala::ImportType::DATAMAP;
            } else {
                throw dmpcli::CommandLineException(std::string("Invalid map import (-it) type: ") +
                                                   argv[i]);
            }
...
void ImportParser::run(const CommandLineParser &clp, IPerformanceSink &perfWriter) const {
    ...
    MetaGraphDM metaGraph("Test mgraph");
    ...
    if (metaGraph.getReadStatus() == MetaGraphReadWrite::ReadWriteStatus::NOT_A_GRAPH) {
        ...
        sala::ImportFileType importFileType = sala::ImportFileType::TSV;
        if (dXstring::toLower(ext) == ".csv") {
            importFileType = sala::ImportFileType::CSV;
        ...
        if (asDrawingLines) {
            ...
            auto newMaps = sala::importFile(file, dm_runmethods::getCommunicator(clp).get(),
                                            clp.getFileName(), getImportMapType(), importFileType);
            if (getImportMapType() == sala::ImportType::DATAMAP) {
                ...
                if (!metaGraph.getDataMaps().empty()) {
                    ...
                    metaGraph.setState(metaGraph.getState() | MetaGraphDM::DX_DATAMAPS);
                    metaGraph.setViewClass(MetaGraphDM::DX_SHOWHIDESHAPE);
                }
            } else {
                auto newDrawingFile =
                    metaGraph.addDrawingFile(clp.getFileName(), std::move(newMaps));
                metaGraph.setState(metaGraph.getState() | MetaGraphDM::DX_LINEDATA);
                if (mimicVersion.has_value() && *mimicVersion == "depthmapX 0.8.0") {
                    // this version does not actually set the map type of the space pixels
                    for (auto &map : metaGraph.getDrawingFiles()[newDrawingFile].maps) {
                        map.getInternalMap().setMapType(ShapeMap::EMPTYMAP);
        ...
        if (toImportAsAttrbiutes()) {
            ...
            if (metaGraph.getDisplayedMapType() == ShapeMap::EMPTYMAP) {
                throw genlib::RuntimeException("No map displayed to attach attributes to");
            }
            ...
            for (std::string &fileName : fileNames) {
                ...
                DO_TIMED(
                    ...
                    sala::importAttributes(metaGraph.getDisplayedMapAttributes(), file, delimiter);)
            }
        }
    }
