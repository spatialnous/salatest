...
void StepDepthParser::parse(size_t argc, char **argv) {
    ...
    for (size_t i = 1; i < argc; ++i) {
        if (std::strcmp("-sdp", argv[i]) == 0) {
            if (!pointFile.empty()) {
                throw dmpcli::CommandLineException("-sdp cannot be used together with -sdf");
            }
            ...
            if (!points.empty()) {
                throw dmpcli::CommandLineException("-sdf cannot be used together with -sdp");
            }
    ...
    if (pointFile.empty() && points.empty()) {
        throw dmpcli::CommandLineException("Either -sdp or -sdf must be given");
    }
    ...
    if (m_stepType == StepType::NONE) {
        throw dmpcli::CommandLineException("Step depth type (-sdt) must be provided");
    }
}
