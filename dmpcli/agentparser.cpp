...
void AgentParser::parse(size_t argc, char *argv[]) {
    ...
    for (size_t i = 1; i < argc;) {
        ...
        if (std::strcmp("-am", argv[i]) == 0) {
            ...
            if (std::strcmp(argv[i], "standard") == 0) {
                ...
                throw dmpcli::CommandLineException(std::string("Invalid AGENTS mode: ") + argv[i]);
            }
        } else if (std::strcmp(argv[i], "-ats") == 0) {
            if (m_totalSystemTimestemps > 0) {
                throw dmpcli::CommandLineException("-ats can only be used once");
            }
            ENFORCE_ARGUMENT("-ats", i)
            if (!dmpcli::has_only_digits(argv[i])) {
                throw dmpcli::CommandLineException(std::string("-ats must be a number >0, got ") +
                                                   argv[i]);
            }
            ...
            if (m_releaseRate > 0) {
                throw dmpcli::CommandLineException("-arr can only be used once");
            }
            ENFORCE_ARGUMENT("-arr", i)
            if (!dmpcli::has_only_digits_dots_commas(argv[i])) {
                throw dmpcli::CommandLineException(std::string("-arr must be a number >0, got ") +
                                                   argv[i]);
            }
            ...
            if (m_recordTrailsForAgents >= 0) {
                throw dmpcli::CommandLineException("-atrails can only be used once");
            }
            ...
            if (m_agentFOV > 0) {
                throw dmpcli::CommandLineException("-afov can only be used once");
            }
            ...
            if (m_agentStepsBeforeTurnDecision > 0) {
                throw dmpcli::CommandLineException("-asteps can only be used once");
            }
            ENFORCE_ARGUMENT("-asteps", i)
            if (!dmpcli::has_only_digits(argv[i])) {
                throw dmpcli::CommandLineException(
                    std::string("-asteps must be a number >0, got ") + argv[i]);
            }
            ...
            if (m_agentLifeTimesteps > 0) {
                throw dmpcli::CommandLineException("-alife can only be used once");
            }
            ENFORCE_ARGUMENT("-alife", i)
            if (!dmpcli::has_only_digits(argv[i])) {
                throw dmpcli::CommandLineException(std::string("-alife must be a number >0, got ") +
                                                   argv[i]);
            }
            ...
            if (!points.empty()) {
                throw dmpcli::CommandLineException("-alocseed cannot be used together with -aloc");
            }
            ...
            if (!dmpcli::has_only_digits(argv[i])) {
                ...
                throw dmpcli::CommandLineException(message.str().c_str());
            }
            ...
            if (!points.empty()) {
                throw dmpcli::CommandLineException("-alocfile cannot be used together with -aloc");
            }
            ...
            if (!pointFile.empty()) {
                throw dmpcli::CommandLineException("-aloc cannot be used together with -alocfile");
            }
            if (m_randomReleaseLocationSeed > -1) {
                throw dmpcli::CommandLineException("-aloc cannot be used together with -alocseed");
            }
    ...
    if (m_releaseRate == 0) {
        throw dmpcli::CommandLineException("Release rate (-arr <rate>) is required");
    }

    if (m_agentFOV == 0) {
        throw dmpcli::CommandLineException("Agent field-of-view (-afov <bins>) is required");
    }
    ...
    if (pointFile.empty() && points.empty() && m_randomReleaseLocationSeed == -1) {
        throw dmpcli::CommandLineException("Either -aloc, -alocfile or -alocseed must be given");
    }
